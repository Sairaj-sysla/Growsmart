...
export class FileUtils {
  ...
  // keep logResult for any internal usage — now unused but kept for safety
  private static logResult(type: string, result: VerifyResult, detail: string): void {
    const kw  = result.containsAll ? "keywords: ✅" : `missing: ${result.missingWords.join(", ")}`;
    const col = result.columnValid  ? "columns: ✅"  : `col errors: ${result.columnErrors.length}`;
    if (result.containsAll && result.columnValid) logger.pass(`${type} valid → ${detail} | ${kw} | ${col}`);
    else                                          logger.warn(`${type} issues → ${detail} | ${kw} | ${col}`);
  }
}